...
describe('Image upload limits', () => {
  ...
  const upload = (files: Array<{ content: Buffer; name: string; type: string }>): Promise<Response> => {
    ...
    files.forEach(({ content, name, type }) => form.append(IMAGE_UPLOAD_LIMITS.FIELD_NAME, new Blob([content], { type }), name));
  ...
  it('rejects more files than allowed with 400', async () => {
    const files = Array.from({ length: IMAGE_UPLOAD_LIMITS.MAX_FILES + 1 }, (_, index) => ({ content: Buffer.from('x'), name: `f${index}.png`, type: 'image/png' }));
