...
import { SupportsPayout } from '../src/modules/interfaces/supports-payout.interface';
import { SupportsWebhooks } from '../src/modules/interfaces/supports-webhooks.interface';
import { PayoutFundsDto } from '../src/modules/dtos/operation/payout-funds.dto';
import { ProviderChargeResultDto } from '../src/modules/dtos/operation/provider-charge-result.dto';
import { WebhookEventDto } from '../src/modules/dtos/operation/webhook-event.dto';
import { ExtractSignatureDto } from '../src/modules/dtos/contract/extract-signature.dto';
import { ConstructWebhookEventDto } from '../src/modules/dtos/contract/construct-webhook-event.dto';
...
describe('extending the registry with a new provider', () => {
  ...
  it('refuses a capability the provider does not declare, instead of crashing later', () => {
    expect(() => registry().require({ providerName: PaymentProvider.ADYEN, capability: PaymentCapability.HOSTED_SETUP })).toThrow(
      /does not support HOSTED_SETUP/
    );

    expect(() => registry().require({ providerName: PaymentProvider.ADYEN, capability: PaymentCapability.CHARGE })).toThrow(
      /does not support CHARGE/
    );
  });
