...
import { RabbitmqService } from '../modules/services/rabbitmq.service';
import { RABBITMQ_CONSTANTS } from '../modules/constants/messaging/rabbitmq.constant';
...
const buildService = async ({ publish }: { publish: PublishMock }): Promise<RabbitmqService> => {
  const channel = { assertExchange: jest.fn().mockResolvedValue(undefined), prefetch: jest.fn().mockResolvedValue(undefined), publish, close: jest.fn().mockResolvedValue(undefined) };
...
describe('RabbitmqService.publish', () => {
  ...
  it('resolves only once the broker confirms the message', async () => {
    ...
    expect(publish).toHaveBeenCalledWith(RABBITMQ_CONSTANTS.RABBITMQ_EXCHANGE.key, ROUTING_KEY, Buffer.from(JSON.stringify(PAYLOAD)), { persistent: true }, expect.any(Function));
  });
